from nowcoder import get_nowcoder
...
class Main:
    ...
    def run():
        res1 = Main.get_upcoming_contests()
        res = Main.filter_today_competition(res1)
        
        if not res:
            print("今天没有即将开始的比赛")
        else:
            for i, contest in enumerate(res, 1):
                print("-" * 60)
        ...
        for i, contest in enumerate(res1, 1):
            print("-" * 60)
            print(f"比赛平台：{contest['platform']}")
            print(f"比赛标题: {contest['title']}")
            print(f"比赛时间: {contest['time']}")
